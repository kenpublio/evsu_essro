 ...
 */
public function submitRegistrarEvaluationWithService($user_id, $service_type_id, $ratings, $answers = [], $report = '') {
    try {
        $registrar = $this->getRegistrarOffice();
        $office_id = $registrar['id'];
        
        // Check if survey is active for this service
        if (!$this->isServiceSurveyActive($service_type_id)) {
            return ['success' => false, 'message' => 'Survey is currently inactive for this service.'];
        }
        
        // Check if user already evaluated THIS service
        if ($this->hasUserEvaluatedService($user_id, $service_type_id)) {
            return ['success' => false, 'message' => 'You have already evaluated this service.'];
        }
        
        $this->conn->begin_transaction();
        
        // Get questions ONLY for the selected service type
        $stmt = $this->conn->prepare("SELECT id, question_text FROM survey_questions WHERE service_type_id = ? AND is_active = 1 ORDER BY display_order");
        $stmt->bind_param("i", $service_type_id);
        $stmt->execute();
        $questions = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        
        // Insert new ratings - ONLY for selected service
        if ($this->tableExists('responses')) {
            ...
            foreach ($questions as $index => $question) {
                ...
                $insert_stmt->bind_param("iiisss", $user_id, $office_id, $service_type_id, $question_text, $rating, $answer);
                $insert_stmt->execute();
                $counter++;
            }
        }
        ...
        return [
            'success' => true,
            'message' => 'Thank you for evaluating the Registrar\'s Office ' . $service_type_id . ' service!',
            'count' => $counter
        ];
...
public function isServiceSurveyActive($service_type_id) {
    $conn = getDB();
    $stmt = $conn->prepare("SELECT is_active, start_date, end_date FROM survey_availability WHERE service_type_id = ?");
    $stmt->bind_param("i", $service_type_id);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($row = $result->fetch_assoc()) {
        $today = date('Y-m-d');
        return $row['is_active'] && $today >= $row['start_date'] && $today <= $row['end_date'];
    }
    return false;
}

public function hasUserEvaluatedService($user_id, $service_type_id) {
    $conn = getDB();
    $stmt = $conn->prepare("SELECT id FROM responses WHERE user_id = ? AND service_type_id = ? LIMIT 1");
    $stmt->bind_param("ii", $user_id, $service_type_id);
    $stmt->execute();
...
public function isSurveyActive() {
    ...
    public function getAdminStats() {
        ...
        // Total evaluations (unique submissions)
        $result = $conn->query("SELECT COUNT(DISTINCT CONCAT(user_id, '-', DATE(submitted_at), '-', IFNULL(service_type_id, 0))) as count FROM responses");
        $stats['total_evaluations'] = $result->fetch_assoc()['count'] ?? 0;
        
        // Average rating
        $result = $conn->query("SELECT AVG(rating) as avg FROM responses WHERE rating > 0");
        $stats['average_rating'] = round($result->fetch_assoc()['avg'] ?? 0, 1);
        
        // Total admins
        $result = $conn->query("SELECT COUNT(*) as count FROM users WHERE role = 'admin'");
        $stats['total_admins'] = $result->fetch_assoc()['count'] ?? 0;
        
        // Active today (users who submitted evaluations in last 24 hours)
        $result = $conn->query("SELECT COUNT(DISTINCT user_id) as count FROM responses WHERE submitted_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)");
        $stats['active_today'] = $result->fetch_assoc()['count'] ?? 0;
        
        // Logs today
        $result = $conn->query("SELECT COUNT(*) as count FROM user_logs WHERE DATE(created_at) = CURDATE()");
        $stats['logs_today'] = $result->fetch_assoc()['count'] ?? 0;
        
        // Top office
        $result = $conn->query("SELECT st.name, AVG(r.rating) as avg_rating 
            FROM responses r 
            ...
            ORDER BY avg_rating DESC 
            LIMIT 1");
        $stats['top_office'] = $result->fetch_assoc() ?? ['name' => 'N/A', 'avg_rating' => 0];
