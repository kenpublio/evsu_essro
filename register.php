...
// Function to validate EVSU email
function isValidEVSUEmail($email) {
    // Allowed EVSU email domains
    $allowed_domains = [
        '@evsu.edu.ph',
...
// Add enrollment_status column to users table if it doesn't exist
...
// Create survey_availability table if it doesn't exist with service_type_id
$conn->query("
...
if ($check['count'] == 0) {
    $services_list = $conn->query("SELECT id FROM service_types");
    while ($service = $services_list->fetch_assoc()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['register'])) {
        ...
        if (empty($username) || empty($password) || empty($confirm_password) || empty($email) || empty($student_id) || empty($fullname)) {
            ...
            $error = "Only EVSU email accounts (@evsu.edu.ph, @students.evsu.edu.ph) are allowed for registration!";
            ...
            if ($check_username->get_result()->num_rows > 0) {
                ...
                if ($check_email->get_result()->num_rows > 0) {
                    ...
                    if ($check_student->get_result()->num_rows > 0) {
                        ...
                        // Register with enrollment status
                        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                        $stmt = $conn->prepare("INSERT INTO users (username, password, email, role, student_id, fullname, is_active, enrollment_status, created_at) VALUES (?, ?, ?, 'student', ?, ?, 1, 'enrolled', NOW())");
                        ...
                        if ($stmt->execute()) {
                            ...
                            $conn->query("INSERT INTO user_logs (user_id, action, ip_address, user_agent) VALUES ($user_id, 'registration', '$ip', '$agent')");
            ...
            <div class="card-body">
                ...
                <?php if (!$success): ?>
                    <form method="POST" action="" id="registerForm">
                        ...
                        <div class="form-group">
                            <label class="form-label">
                                <i class="fas fa-envelope"></i>Evsu Email Address *
                            </label>
                            <input type="email" 
                                   name="reg_email" 
                                   id="regEmail" 
                                   ...
                                   required>
                            <span class="form-hint">Use your EVSU email address</span>
                        </div>
