...
// Function to validate EVSU email only
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['login'])) {
        ...
        if (empty($student_id) || empty($password)) {
            ...
            // Check login credentials using student_id
            $stmt = $conn->prepare("SELECT id, username, password, role, fullname, is_active, enrollment_status FROM users WHERE student_id = ?");
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                $ip = $_SERVER['REMOTE_ADDR'] ?? 'Unknown';
                $agent = $_SERVER['HTTP_USER_AGENT'] ?? 'Unknown';
                $conn->query("INSERT INTO user_logs (user_id, action, ip_address, user_agent) VALUES ({$user['id']}, 'login_success', '$ip', '$agent')");
                
                // Redirect based on role
               // Redirect all users to student dashboard (admin can access via footer link)
    ...
    if (isset($_POST['register'])) {
        ...
        if (empty($username) || empty($password) || empty($confirm_password) || empty($email) || empty($student_id) || empty($fullname)) {
            ...
            $showRegister = true;
        } elseif (!isValidEVSUEmail($email)) {
            $error = "Only EVSU email accounts (@evsu.edu.ph, @students.evsu.edu.ph, @evsu-occ.edu.ph) are allowed for registration!";
            $showRegister = true;
            ...
            if ($check_username_stmt->get_result()->num_rows > 0) {
                ...
                if ($check_email_stmt->get_result()->num_rows > 0) {
                    ...
                    if ($check_student_stmt->get_result()->num_rows > 0) {
                        ...
                        if ($result['success']) {
                            $success = "Registration successful! Please login with your Student ID and Password.";
                            $showRegister = false; // Switch to login form
                            
                            // Pre-fill the student_id in login form
                ...
                <form method="POST" action="" class="auth-form <?php echo !$showRegister ? 'active' : ''; ?>" id="loginForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="student_id" name="student_id" class="form-control" required 
                               ...
                               placeholder="e.g., 2023-12345" pattern="\d{4}-\d{5}">
                ...
                <form method="POST" action="" class="auth-form <?php echo $showRegister ? 'active' : ''; ?>" id="registerForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="reg_student_id" name="reg_student_id" class="form-control" required 
                               ...
                               placeholder="e.g., 2023-12345" pattern="\d{4}-\d{5}">
                        <span class="form-hint">Enter your official EVSU student ID (Format: YYYY-XXXXX)</span>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <span class="form-hint">Only EVSU email accounts are allowed (@evsu.edu.ph)</span>
                    </div>
                    ...
                    <div class="form-footer">
                        ...
                        <a href="student/index.php" onclick="showForm('login')" class="form-link">
                            Sign in here
                        </a>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (registerForm) {
                registerForm.addEventListener('submit', function(e) {
                    ...
                    // Check email domain
                    const allowedDomains = ['@evsu.edu.ph', '@students.evsu.edu.ph', '@evsu-occ.edu.ph'];
                    ...
                    if (!isValidDomain) {
                        e.preventDefault();
                        alert('Only EVSU email accounts (@evsu.edu.ph) are allowed for registration!');
                    ...
                    if (!studentIdPattern.test(studentId)) {
                        e.preventDefault();
                        alert('Student ID must be in format: YYYY-XXXXX (e.g., 2023-12345)');
